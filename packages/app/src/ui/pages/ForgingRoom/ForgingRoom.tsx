...
import {
  getAvailableForgingRecipes, canForgeRecipe, getForgingRate, getForgingBonus,
} from '@/data/forging';
import { getItem } from '@/data/items';
import type { ForgingRecipe } from '@/data/forging';

export const ForgingRoom: React.FC = () => {
  ...
  const [selected, setSelected] = useState<ForgingRecipe | null>(null);
  const [crafting, setCrafting] = useState<string | null>(null);

  if (!player) return null;

  const recipes = getAvailableForgingRecipes(player.realm);
  const bonus = getForgingBonus();

  const handleForge = (recipe: ForgingRecipe) => {
    setCrafting(recipe.id);
    setTimeout(() => {
      const { ok, missing } = canForgeRecipe(recipe, player.inventory, player.gold);
      if (!ok) {
      ...
      const rate = getForgingRate(recipe);
      ...
      if (success) {
        ...
        if (resultItem) addItem(resultItem, 1);
        window.dispatchEvent(new CustomEvent('notification', {
          detail: { type: 'success', message: `炼器成功！获得【${resultItem?.name || recipe.name}】！` },
        }));
      } else {
        window.dispatchEvent(new CustomEvent('notification', {
          detail: { type: 'error', message: '炼器失败！材料已废，熔炉冷却中…' },
        }));
      }
      setCrafting(null);
    }, 1500);
    ...
    <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontWeight: 600, fontSize: 16 }}>⚒️ 炼器室</div>
      ...
      <div className="panel" style={{ padding: 10, marginBottom: 12, fontSize: 12, color: 'var(--text-secondary)' }}>
        💡 炼器失败会损失全部材料，建议优先提升炼器室等级
      </div>

      {/* 配方列表 */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {recipes.map((recipe) => {
          const { ok } = canForgeRecipe(recipe, player.inventory, player.gold);
          const rate = Math.round(getForgingRate(recipe) * 100);
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  {resultItem && (
                    <span style={{
                      marginLeft: 6, fontSize: 11, color: 'var(--text-tertiary)',
                    }}>
                      {recipe.description}
                    </span>
                  )}
              ...
              {selected?.id === recipe.id && (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border-default)' }}>
                  <div style={{ fontSize: 12, marginBottom: 8, color: 'var(--text-tertiary)' }}>
                    境界要求：{recipe.minRealm}
                    {resultItem && (
                      <>
